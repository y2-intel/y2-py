...
from y2 import Y2, AsyncY2, DefaultAioHttpClient
from y2._utils import is_dict
...
logging.getLogger("y2").setLevel(logging.DEBUG)
...
def client(request: FixtureRequest) -> Iterator[Y2]:
    strict = getattr(request, "param", True)
    if not isinstance(strict, bool):
        raise TypeError(f"Unexpected fixture parameter type {type(strict)}, expected {bool}")

    with Y2(base_url=base_url, api_key=api_key, _strict_response_validation=strict) as client:
        yield client


@pytest.fixture(scope="session")
async def async_client(request: FixtureRequest) -> AsyncIterator[AsyncY2]:
